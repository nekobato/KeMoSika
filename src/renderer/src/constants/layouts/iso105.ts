...
const numpadTopRow = createRow(prefix, 82, 1150, [
...
const isoEnter = createKeyboardKey(prefix, {
  id: "enter",
  label: "Enter",
  codeMap: ["enter"],
  width: 76,
...
const numpadMiddleTopRow = createRow(prefix, 144, 1150, [
...
const numpadMiddleRow = createRow(prefix, 206, 1150, [
...
const zRow = createRow(prefix, 268, 20, [
  ...
  key("period", ".", ["."]),
  key("slash", "/", ["/"]),
  key("shift-right", "Shift", ["shift"], 154)
]);

const arrowUpRow = createRow(prefix, 300, 990, [
  key("arrow-up", "↑", ["up"])
], 44);

const numpadLowerRow = createRow(prefix, 268, 1150, [
...
const arrowBottomRow = createRow(prefix, 348, 934, [
  key("arrow-left", "←", ["left"]),
  key("arrow-down", "↓", ["down"]),
  key("arrow-right", "→", ["right"])
], 44);

const numpadBottomRow = createRow(prefix, 330, 1150, [
  key("numpad0", "0", ["numpad0"], 112),
  key("numpad-decimal", ".", ["numpad."])
]);

export const iso105Layout: LayoutData = createLayout({
  id: "builtin-iso-105",
  name: "ISO 105キー",
  width: 1420,
  height: 420,
